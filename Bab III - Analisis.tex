...
\section{Karakteristik dan Deskripsi Dataset Penelitian}

Kinerja dan keandalan model hibrida sekuensial yang dikembangkan dalam penelitian ini sangat bergantung pada kualitas dan karakteristik data mentah yang digunakan. Bagian ini menguraikan deskripsi dataset secara eksplisit, meliputi durasi pencatatan, cakupan data pengamatan, jumlah sampel, serta variabel-variabel atmosferik yang terlibat sebagai pemenuhan parameter kelayakan pengolahan data sains. Dataset yang digunakan dalam penelitian ini merupakan data sekunder berupa rekaman historis Indeks Standar Pencemar Udara (ISPU) yang diperoleh secara resmi melalui Dinas Lingkungan Hidup Provinsi DKI Jakarta.

\subsection{Durasi dan Cakupan Data Pengamatan}

Pengamatan kualitas udara dalam penelitian ini mencakup dimensi temporal jangka panjang dengan frekuensi pengamatan harian (\textit{daily time-series}) yang memiliki durasi total selama 16 tahun, terhitung sejak tanggal 1 Januari 2010 hingga 31 Desember 2025. Data pengamatan tersebut dikumpulkan secara spasial dari lima Stasiun Pemantau Kualitas Udara Ambien (SPKUA) yang tersebar secara strategis di berbagai wilayah administratif Provinsi DKI Jakarta. Kelima stasiun tersebut meliputi Stasiun DKI1 yang berlokasi di kawasan bisnis Bundaran HI (Jakarta Pusat), Stasiun DKI2 di kawasan logistik Kelapa Gading (Jakarta Utara), Stasiun DKI3 di kawasan permukiman hijau Jagakarsa (Jakarta Selatan), Stasiun DKI4 di kawasan padat penduduk Lubang Buaya (Jakarta Timur), dan Stasiun DKI5 di koridor transportasi Kebon Jeruk (Jakarta Barat). Pemilihan kelima lokasi ini bertujuan untuk menangkap variabilitas spasial dan karakteristik emisi lokal yang bervariasi di seluruh wilayah metropolitan.

\subsection{Jumlah Data dan Spesifikasi Variabel}

Secara total, dengan rentang waktu harian selama 16 tahun pencatatan yang turut mengasimilasi perhitungan tahun kabisat, dataset ini menghasilkan akumulasi data mentah sebanyak 5.844 baris observasi kontinu untuk masing-masing stasiun pemantau. Ketika seluruh data spasial tersebut digabungkan ke dalam satu kesatuan matriks komputasi multidimensi, sistem mengidentifikasi total keseluruhan sampel data pengamatan sebanyak 29.220 baris observasi harian. 

Seluruh baris observasi tersebut disusun oleh beberapa komponen variabel teknis yang terbagi menjadi dua kategori utama, yaitu variabel identitas data dan variabel polutan atmosferik. Variabel identitas terdiri atas kolom \texttt{tanggal} yang bertindak sebagai indeks waktu runtun waktu kronologis, serta kolom \texttt{stasiun} yang berfungsi sebagai kode penanda lokasi fisik pengamatan spasial. Sementara itu, dimensi masukan multivariat dibentuk oleh lima variabel parameter polutan utama yang mencakup PM\textsubscript{10} sebagai representasi dari konsentrasi partikulat mikro (\textit{Particulate Matter} $\le$ 10 $\mu m$), SO\textsubscript{2} (Sulfur Dioksida), PM\textsubscript (Karbon Monoksida), O\textsubscript{3} (Ozon Permukaan), dan NO\textsubscript{2} (Nitrogen Dioksida). Integrasi dari seluruh komponen kuantitatif dan spesifikasi variabel ini secara fisis merepresentasikan kondisi polusi udara riil di DKI Jakarta yang siap digunakan untuk melatih arsitektur model hibrida sekuensial.
...
Komponen $L_t$ pada Persamaan \ref{eq:formalisasi2} merepresentasikan fungsi linier yang memiliki sifat autokorelasi, di mana komponen ini dimodelkan menggunakan persamaan diferensi stokastik ARIMA. Sementara itu, komponen $N_t$ mewakili fungsi non-linier—seperti pengaruh interaksi antarpolutan dan variasi meteorologi—yang dipetakan secara non-parametrik melalui struktur ensambel pohon keputusan \textit{Random Forest} Regresi. 
...
Proses ekstraksi ini memetakan nilai historis masa lalu menjadi fitur prediktor independen pada matriks pelatihan. Fitur yang diekstraksi meliputi konsentrasi polutan pada satu hari sebelumnya ($\text{PM}_{10}(t-1)$) hingga tujuh hari sebelumnya ($\text{PM}_{10}(t-7)$). Formulasi matematis dari pembentukan \textit{lag features} ditunjukkan pada Persamaan \ref{eq:lag_features}.
...
Penerapan fitur jeda waktu atau \textit{lag} ini berfungsi untuk memasukkan nilai historis kualitas udara dari hari-hari sebelumnya ke dalam model. Sebagai contoh, fitur \textit{lag}-1 menggunakan data konsentrasi polutan satu hari sebelumnya, sedangkan fitur \textit{lag}-7 menggunakan data dari tujuh hari sebelumnya untuk membantu model mempelajari kecenderungan pengulangan pola mingguan berdasarkan riwayat pencatatan yang ada.
...
Kombinasi dari metrik statistik ini memungkinkan model \textit{Random Forest} Regresi untuk memahami apakah data observasi harian berada dalam kondisi normal, sedang mengalami tren perubahan nilai, atau merupakan fluktuasi sesaat. Kalkulasi matematis untuk masing-masing fitur statistik jendela berjalan diturunkan pada Persamaan \ref{eq:roll_mean3}, Persamaan \ref{eq:roll_mean7}, dan Persamaan \ref{eq:roll_std7}.
...
Perkalian komposit antara konsentrasi Karbon Monoksida (CO) dan Ozon ($O_3$) digunakan sebagai parameter tambahan untuk membantu model dalam mengidentifikasi kondisi atmosfer. Gas CO mencerminkan kerapatan emisi dari aktivitas transportasi harian, sedangkan gas $O_3$ di permukaan tanah berkaitan dengan paparan sinar matahari harian. Kombinasi kedua nilai ini memberikan informasi struktural mengenai interaksi antargas prekursor yang memengaruhi keberadaan partikulat di udara terbuka.
...
\hline
2 & F-02 & Mengeksekusi peramalan melalui implementasi arsitektur hibrida sekuensial menggabungkan \textit{Random Forest} Regresi (RF) dan ARIMA. \\
\hline
...
Oleh karena itu, kerangka kerja AHP pada penelitian ini tidak diposisikan sebagai pengganti evaluasi kinerja empiris model akhir, melainkan sebagai instrumen analisis kebutuhan pra-eksperimen (\textit{pre-development framework}) untuk menjustifikasi pemilihan paradigma arsitektur hibrida berbasis studi Yenkikar dkk. \autocite{yenkikar2025explainable} sebagai jangkar utama. Kerangka hibrida sekuensial Yenkikar dkk. dinilai sangat ideal karena secara teoritis mampu menyeimbangkan kriteria akurasi dan interpretabilitas via XAI \autocite{yenkikar2025explainable}. Langkah analisis ini diwujudkan dengan menyusun matriks perbandingan berpasangan (\textit{pairwise comparison}) untuk lima kriteria operasional, yaitu Interpretabilitas (INT), Akurasi (AKR), Kemampuan Menangkap Pola Kompleks (PLK), Efisiensi Komputasi (KMP), dan Kemudahan Implementasi (IMP).
...
Berdasarkan nilai tersebut, model statistik linier ARIMA menghasilkan nilai lokal tertinggi (0,667) karena hubungannya dipetakan langsung melalui koefisien matematika yang transparan. Model \textit{Random Forest} Regresi murni memiliki nilai rendah (0,167) karena karakteristiknya yang tertutup (\textit{black-box}). Namun, pada model hibrida usulan, kelemahan sifat tertutup tersebut diatasi dengan mengintegrasikan modul analisis SHAP pada tahapan pascapelatihan. Intervensi ini memungkinkan model hibrida memberikan kejelasan kontribusi fitur, sehingga bobot lokal kriteria ini dapat dipertahankan untuk mendukung nilai prioritas global.
...
\hline
RF Regresi & 0,167 & 0,258 & 0,327 & 0,444 & 0,429 & \textbf{0,271} (27,1\%) \\
\hline
