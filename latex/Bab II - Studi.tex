...
\hline
\textcite{alarsy2025pm25} & \textit{Random Forest} Regresi & Polutan mentah & Mengabaikan tren linier jangka panjang. \\
\hline
...
Di antara berbagai literatur yang dievaluasi, studi oleh Yenkikar dkk. \autocite{yenkikar2025explainable} dipilih sebagai referensi utama dalam Tugas Akhir ini karena berhasil mendemonstrasikan kerangka komplementer hibrida sekuensial yang kokoh. Namun, karena adanya keterbatasan model dasar tersebut yang mengabaikan hubungan fotokimiawi lokal, penelitian ini membatasi kontribusinya pada aspek analisis pengaruh fitur baru dan evaluasi performa arsitektur tersebut di wilayah DKI Jakarta. 
...
Pembuatan fitur turunan baru yang merupakan hasil kali interaksi antara gas tersebut ($CO \times O_3$) bertujuan untuk mendeteksi kondisi atmosferik yang reaktif tersebut. Secara komputasi, fitur tambahan ini memberikan informasi varians tambahan kepada algoritma \textit{Random Forest} Regresi untuk mengenali pola pembentukan polusi sekunder. Penggunaan fitur interaksi kimiawi ini ditujukan untuk meminimalkan kesalahan prediksi pada saat terjadinya lonjakan polusi mendadak \autocite{naz2024two}. Dengan demikian, model memasukkan representasi karakteristik fisik dan kimia yang terjadi di atmosfer perkotaan.
...
Untuk memastikan apakah sebuah data sudah stasioner, pengujian dilakukan secara statistik menggunakan uji \textit{Augmented Dickey-Fuller} (ADF) \autocite{vasconcelos2025stationarity}. Uji ADF bekerja dengan mendeteksi ada tidaknya akar unit (\textit{unit root}) pada data. Evaluasi menggunakan model regresi yang diturunkan pada Persamaan \ref{eq:adf}.
...
Secara teknis, mekanisme ini bekerja dengan membagi data latih menjadi beberapa bagian kecil yang berbeda untuk diproses oleh setiap pohon keputusan. Ketika melakukan estimasi, setiap pohon keputusan akan mengeluarkan hasil perhitungan sendiri, kemudian seluruh hasil tersebut dirata-ratakan untuk mendapatkan satu angka estimasi akhir. Pendekatan ini digunakan untuk meminimalkan ketergantungan model pada satu variabel tertentu dan menjaga kestabilan hasil prediksi.
...
\label{eq:rf_math}
\end{equation}
\eqcaption{Formulasi Agregasi Prediksi Akhir Ansambel \textit{Random Forest} Regresi}
...
\begin{enumerate}
   \item {Fase Primer:} Model \textit{Random Forest} Regresi (RF) digunakan pada tahap awal untuk memproses variabel fitur masukan dan menghasilkan proyeksi estimasi nilai awal ($\hat{N}_t$). RF bertugas menangkap komponen tren non-linier data polusi.
...
Meskipun model ansambel seperti \textit{Random Forest} Regresi memberikan nilai akurasi estimasi yang terkendali, model ini memiliki karakteristik komputasi yang tertutup (\textit{black-box}) \autocite{jimenez2024explainable}. Untuk kebutuhan analisis polusi udara, model harus mampu memberikan penjelasan mengenai variabel fitur yang memengaruhi variasi nilai konsentrasi PM\textsubscript{10}. Penelitian ini menerapkan pendekatan \textit{Explainable AI} (XAI) melalui metode SHAP (\textit{SHapley Additive exPlanations}) untuk memetakan besaran pengaruh setiap variabel masukan terhadap hasil estimasi akhir.
...
\subsection{Kerangka Pemikiran Aliran Data-ke-Luaran}
\label{subsection:kerangka_transformasi}

Untuk memberikan landasan teoretis yang terstruktur mengenai proses perubahan data mentah hingga menjadi luaran prediktif yang dapat ditelaah, disusun sebuah kerangka konseptual aliran data. Tahapan ini menggambarkan bagaimana domain sains data dan sains atmosfer disintesis secara berurutan tanpa mendahului pembahasan perancangan sistem fisik. Proses transformasi tersebut dibagi menjadi empat fase logis utama:

\begin{enumerate}
   \item {Fase Akuisisi dan Pengayaan Masukan (\textit{Input Phase}):} 
   Data mentah yang terdiri atas variabel polutan dasar univariat diperluas dimensi informasinya melalui rekayasa fitur dimensi atmosferik. Pengolahan ini menghasilkan variabel baru berupa komponen \textit{lag temporal} untuk persistensi, metrik tren berjalan untuk stabilisasi derau, serta fitur komposit interaksi kimiawi ($CO \times O_3$) untuk memetakan pembentukan polutan sekunder. Matriks dimensi baru ini kemudian diselaraskan skalanya menggunakan transformasi \textit{Z-Score}.

   \item {Fase Analisis Varians Non-Linier (\textit{Fase Primer}):} 
   Matriks fitur multidimensi yang telah terstandardisasi diproses oleh komponen pembelajar primer, yaitu algoritma \textit{Random Forest} Regresi. Algoritma ini memetakan karakteristik non-linier data melalui pembentukan ansambel pohon keputusan secara non-parametrik. Hasil akhir dari fase ini berupa nilai estimasi awal ($\hat{N}_t$) dan ekstraksi deret sisaan galat temporal ($e_t$) yang belum mampu diurai oleh pemisahan cabang pohon.
   
   \item {Fase Koreksi Dependensi Linier (\textit{Fase Sekunder}):} 
   Deret sisaan galat ($e_t$) diisolasi menjadi runtun waktu univariat satu dimensi untuk dianalisis stasioneritasnya melalui uji ADF. Model statistik ARIMA kemudian diterapkan pada fase sekunder ini untuk menyelesaikan pola autokorelasi linier yang tertinggal pada residu, menghasilkan nilai proyeksi koreksi galat ($\hat{L}_t$).
   
   \item {Fase Agregasi Luaran dan Atribusi Fisis (\textit{Output \& Explanation Phase}):} 
   Kombinasi akhir hasil peramalan diperoleh melalui fungsi jumlahan aditif antara hasil estimasi fase primer dan komponen koreksi fase sekunder ($\hat{Y}_{hybrid}$). Bersamaan dengan pembentukan nilai estimasi tersebut, algoritma \textit{TreeSHAP} mengekstrak nilai kontribusi marjinal dari lima belas fitur masukan pada fase primer. Hasil akhir (*final output*) dikemas dalam format pelaporan yang menyajikan angka estimasi ke depan beserta rincian visualisasi faktor pendorong polusi.
\end{enumerate}
